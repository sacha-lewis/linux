...
split into a small recovery-first bootstrap layer and a separate provisioning
...
Python should not be required for the critical bootstrap path unless the target
OS already guarantees Python 3. The bootstrap layer should stay as close to
...
* `upgrade` updates files, packages, and service definitions without deleting
  local state.
* `verify` checks the machine and reports what is working or missing.

Bootstrap commands should be safe to rerun. Provisioning commands should be safe
...
The bootstrap layer must remain small, stable, and rarely modified.

The bootstrap layer must not install application software.

Software that does not belong in bootstrap:
...
Purpose: convert the machine into a specific role after bootstrap recovery is in
place.
...
of the bootstrap recovery path.
...
* Define the exact bootstrap script entry point.
* Define where bootstrap configuration will live.
* Define the systemd service name and unit file location.
* Define how the reverse SSH tunnel port will be assigned.
* Define what machine information is registered during bootstrap.
...
* Confirm whether Python helpers are allowed outside the bootstrap-critical
  path.
...
bootstrap/
    ...
    register-machine.sh
    systemd/
        digi-display-bootstrap.service
provision.sh
profiles/
...
* Keep bootstrap scripts independent from provisioning scripts.
...
Bootstrap verification should confirm:
...
* Application software does not become a bootstrap dependency.
...
These features belong above the bootstrap layer and should not be required for
machine recovery.
